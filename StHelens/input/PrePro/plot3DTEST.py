...
f4 = open('test0_newDEM_out.asc', 'r')
...
for i in range(len(x)):
	...
	for j in range(len(y)):
		...
		if float(r[j]) > 0.0:
			rm.append(r[j])
		if float(r[j]) < 0.0:
			rm.append(1000.0)# 	rm.append(0.0)
		if float(q[j]) > 0.0:
			qm.append(q[j])
		if float(q[j]) < 0.0:
			qm.append(1000.0)# 	rm.append(0.0)
		if float(p[j]) > 0.0:
			pm.append(p[j])
		if float(p[j]) < 0.0:
			pm.append(1000.0)# 	rm.append(0.0)
...
ax.scatter(axm, aym, apm, color= colour1, s=100)
ax.scatter(axm, aym, aqm, color= colour, s=8)
